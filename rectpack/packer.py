...
SORT_AREA = lambda rectlist: sorted(
    rectlist, reverse=True, key=lambda r: r[0] * r[1]
)  # Sort by area

SORT_PERI = lambda rectlist: sorted(
    rectlist, reverse=True, key=lambda r: r[0] + r[1]
)  # Sort by perimeter

SORT_DIFF = lambda rectlist: sorted(
    rectlist, reverse=True, key=lambda r: abs(r[0] - r[1])
)  # Sort by Diff

SORT_SSIDE = lambda rectlist: sorted(
    rectlist, reverse=True, key=lambda r: (min(r[0], r[1]), max(r[0], r[1]))
)  # Sort by short side

SORT_LSIDE = lambda rectlist: sorted(
    rectlist, reverse=True, key=lambda r: (max(r[0], r[1]), min(r[0], r[1]))
)  # Sort by long side

SORT_RATIO = lambda rectlist: sorted(
    rectlist, reverse=True, key=lambda r: r[0] / r[1]
)  # Sort by side ratio
...
class BinFactory(object):
    ...
    def _create_bin(self):
        return self._pack_algo(
            self._width, self._height, *self._algo_args, **self._algo_kwargs
        )
...
class PackerBBFMixin(object):
    ...
    def add_rect(self, width, height, rid=None, force_rotate=False):
        ...
        fit = ((b.fitness(width, height, force_rotate), b) for b in self._open_bins)
...
class Packer(PackerOnline):
    ...
    def add_rect(self, width, height, rid=None, force_rotate=False):
        self._avail_rect.append((width, height, rid, force_rotate))
...
class PackerGlobal(Packer, PackerBNFMixin):
    ...
    def _find_best_fit(self, pbin):
        ...
        """
        fit = (
            (pbin.fitness(r[0], r[1], r[3]), k) for k, r in self._sorted_rect.items()
        )
